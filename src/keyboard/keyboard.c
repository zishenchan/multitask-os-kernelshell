...
{
    struct process* process = process_current();// we are looking for what we process to
    if (!process)
    {
        return;
    }
...
char keyboard_pop()
{
    ...
    struct process* process = task_current()->process;// when we pop, we need the current task is running
    int real_index = process->keyboard.head % sizeof(process->keyboard.buffer);
    char c = process->keyboard.buffer[real_index];
    if (c == 0x00)// see if the index is 0
    {
