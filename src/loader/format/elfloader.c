...
const char elf_signature[] = {0x7f, 'E', 'L', 'F'};// 0x7f in manual pages 

static bool elf_valid_signature(void* buffer) // check the signature
{
...
{
    return header->e_ident[EI_DATA] == ELFDATANONE || header->e_ident[EI_DATA] == ELFDATA2LSB;// the data encoding in muanual pages
}

static bool elf_is_executable(struct elf_header* header)
{
    return header->e_type == ET_EXEC && header->e_entry >= MULTITASK_OS_KERNELSHELL_PROGRAM_VIRTUAL_ADDRESS;
    // have to check whether load over 0x400000
}
...
{
    return (struct elf32_shdr*)((int)header+header->e_shoff);// make sure (int)
}
...
{
    return elf_memory(file)+phdr->p_offset;// offset from beginning of elf file
}

/**
 * Founction for string table
 */
...
{
    return (elf_valid_signature(header) && elf_valid_class(header) && elf_valid_encoding(header) && elf_has_program_header(header)) ? MULTITASK_OS_KERNELSHELL_ALL_OK : -EINFORMAT;
}


/**
 * install the elf loader part 2
 */
int elf_process_phdr_pt_load(struct elf_file* elf_file, struct elf32_phdr* phdr)
{
    ...
    {
        /**
         * Calculate the virtual base address and physical base address
         */
        elf_file->virtual_base_address = (void*) phdr->p_vaddr;
        elf_file->physical_base_address = elf_memory(elf_file)+phdr->p_offset;// pysical base address
    }

    unsigned int end_virtual_address = phdr->p_vaddr + phdr->p_filesz;// calculation
...
int elf_process_pheader(struct elf_file* elf_file, struct elf32_phdr* phdr)
{
    ...
    switch(phdr->p_type)
    {
        case PT_LOAD:// in elf manual pages
...
int elf_process_pheaders(struct elf_file* elf_file)
{
    ...
    for(int i = 0; i < header->e_phnum; i++) // loop through all program headers
    {
...
int elf_load(const char* filename, struct elf_file** file_out)
{
    ...
    elf_file->elf_memory = kzalloc(stat.filesize);// creat memeory in heap to store elf
    ...
    *file_out = elf_file;// the pointer being given in elf_load()
out:
    fclose(fd);
    return res;
}
