...
{
    int len = strnlen(filename, MULTITASK_OS_KERNELSHELL_MAX_PATH);
...
{
    char* result_path_part = kzalloc(MULTITASK_OS_KERNELSHELL_MAX_PATH);
    int i = 0;
...
struct path_root* pathparser_parse(const char* path, const char* current_directory_path)
{
    ...
    if (strlen(path) > MULTITASK_OS_KERNELSHELL_MAX_PATH)
    {
        goto out;
    }
